/**
 * @file   TVDCTrackingProcessor_las.cc
 * @brief
 *
...
#include "TVDCTrackingProcessor_las.h"
...
using art::TVDCTrackingProcessor_las;

ClassImp(art::TVDCTrackingProcessor_las)

  // Default constructor
  TVDCTrackingProcessor_las::TVDCTrackingProcessor_las()
...
void TVDCTrackingProcessor_las::Init(TEventCollection *col)
{
  //  double x,y,a,b;
...
void TVDCTrackingProcessor_las::Process()
{
...
//  if(debug)getchar();
  ...
  if(debug)  printf("trapro_new 1.5\n");
  int id[4][50][50];
  double dl[4][50][50];
...
//  double	pli[4][6]={{-757.76,0,0,6,0.0,10.},{-825.76,0,20,6,-48.19,10.},{-508/36,0,164,6,0.0,10.},{-576.36,0,184,6,-31,10.}};
//  double	pli[4][6]={{-757.76,38.671,0,6,0.0,10.},{-825.76,38.671,20,6,31,10.},{-508.36,38.671,164,6,0.0,10.},{-576.36,38.671,184,6,31,10.}};
 double	pli[4][6]={{-757.76,0,0,6,0.0,10.},{-825.76,0,20,6,31,10.},{-508.36,0,164,6,0.0,10.},{-576.36,0,184,6,31,10.}};
// double	pli[4][6]={{757.76,0,0,-6,0.0,10.},{825.76,0,20,-6,31,10.},{508.36,0,164,-6,0.0,10.},{576.36,0,184,-6,31,10.}};
...
/*
	 ...
	 double posu1 = (cluster_pos[1][selected_cluster[1]] + pli[1][0]/7.)*7;
	 double posx2 = (cluster_pos[2][selected_cluster[2]] + pli[2][0]/6)*6;
	 double posu2 = (cluster_pos[3][selected_cluster[3]] + pli[3][0]/7)*7;

	 double tempx =posx1;
	 double tempa =(posx2-posx1)/(pli[2][2]-pli[0][2]); 
	 double tempb = (tempa - (posu2-posu1)/(pli[3][2]-pli[1][2]))/tan(31/180.*3.141592);
	 double tempy = (tempx + (pli[1][2]*posu2-pli[3][2]*posu1)/(pli[3][2]-pli[1][2]))/tan(31/180.*3.141592);

	 if(debug)	 printf("x=%lf,y=%lf,a=%lf,b=%lf, atan(a) = %lf deg,atan(b) = %lf deg\n",tempx,tempy,tempa,tempb,atan(tempa)*180/3.14,atan(tempb)*180/3.14);
	 if(debug)	 printf("x2=%lf,y2=%lf,a2=%lf,b2=%lf, atan(a2) = %lf deg,atan(b2) = %lf deg\n",temp2x,temp2y,temp2a,temp2b,atan(temp2a)*180/3.14,atan(temp2b)*180/3.14);
	 */
	 double SSR=0;
	 double sumres=0;
	 ...
	 double res[4][50];
	 for(int i=0;i<4;i++){
		double aangle=pli[i][4]*3.141592/180.;
//		double space = -pli[i][3];
		double space = pli[i][3];
		double kankaku = pli[i][3]/cos(aangle);
		double z= pli[i][2];
...
//res[i][jk] = ((id[i][selected_cluster[i]][jk]+(pli[i][0]/(kankaku)))*kankaku-((tempx+tempa*z)*cos(aangle*3.141592/180.)+(tempy+tempb*z)*sin(aangle*3.141592/180.)))/(tempa*cos(aangle*3.141592/180.)+tempb*sin(aangle*3.141592/180.)) - dl[i][selected_cluster[i]][jk]*10;
//	res[i][jk] = (((tempx+tempa*z)-(id[i][selected_cluster[i]][jk]*kankaku+pli[i][0]))-(tempy+tempb*z)*sin(aangle))/(tempb*sin(aangle)-tempa)-dl[i][selected_cluster[i]][jk]*10;
	res[i][jk] = (((tempx+tempa*z)-(id[i][selected_cluster[i]][jk]*kankaku+pli[i][0]))+(tempy+tempb*z)*tan(aangle))/(-tempb*tan(aangle)-tempa)-dl[i][selected_cluster[i]][jk]*10;
...
//		  if(debug)printf("id=%d,dl=%lf,res[%d][%d]=%lf\n",id[i][selected_cluster[i]][jk],dl[i][selected_cluster[i]][jk]*10,i,jk,res[i][jk]);
  ...
  trackingResult->SetTrack(bestx,besty,0,(atan(besta)-3.14159265*54.04/180.)*180./3.141592,atan(bestb/sqrt(2))/3.141592*180.);//sqrt(2)は回転のぶん
