/**
 * @file   TGMWDCCalibration.cc
 * @brief  MWDC calibration GUI
 *
 * @date   Created       : 2014-04-03 10:00:52 JST
 *         Last Modified : 2014-07-17 09:58:06 JST (kawase)
...
// TGMWDCCalibration
//
// Provides a GUI-based user-friendly calibration tool for BLD-MWDC.
...
using art::TGMWDCCalibration;

ClassImp(TGMWDCCalibration);

TString art::TGMWDCCalibration::fgMWDCParamDir = "prm/mwdc";
TString art::TGMWDCCalibration::fgCalibParamSubDir = "dt2dl";

TGMWDCCalibration::TGMWDCCalibration(const TGWindow *p, UInt_t w, UInt_t h, TObjArray *a)
   : fHt(NULL), fHtl(NULL), fHists(a), fLastDir(gSystem->ConcatFileName(gSystem->pwd(),fgMWDCParamDir))
{
   // constructor
   if (!a) return;
...
TGMWDCCalibration::~TGMWDCCalibration()
{
   if (fMain) fMain->CloseWindow();
   delete fHtl;
}

void TGMWDCCalibration::Build(const TGWindow *p, UInt_t w, UInt_t h)
{
   // Build main frame
   ...
   fMain->SetClassHints("ROOT","art::TGMWDCCalibration");
   ...
   fComboMWDC = new TGComboBox(fcombo,id++);
   fComboMWDC->Resize(75,20);
   fcombo->AddFrame(fComboMWDC, new TGLayoutHints(kLHintsLeft | kLHintsCenterY,2,2,2,2));
   ...
   fBSaveMWDC = new TGPictureButton(fcombo,gClient->GetPicture("fileopen.xpm"),id++);
   fcombo->AddFrame(fBSaveMWDC, new TGLayoutHints(kLHintsCenterY,2,2,2,2));
   ...
   fBSaveMWDC->SetToolTipText("Save parameter files for a MWDC at the same time");
   prev->SetToolTipText("previous plane");
   next->SetToolTipText("next plane");

   /* signal & slot */
   fNEMin->Connect("ReturnPressed()","art::TGMWDCCalibration",this,"UpdateByNE()");
   fNEMin->Connect("ReturnPressed()","art::TGMWDCCalibration",this,"DoDraw()");
   fNEMin->Connect("TabPressed()","TGNumberEntryField",fNEMax,"RequestFocus()");
   fNEMax->Connect("ReturnPressed()","art::TGMWDCCalibration",this,"UpdateByNE()");
   fNEMax->Connect("ReturnPressed()","art::TGMWDCCalibration",this,"DoDraw()");
   fNEMax->Connect("TabPressed()","TGNumberEntryField",fNEMin,"RequestFocus()");

   fBXval->Connect("Clicked()","art::TGMWDCCalibration",this,"SetRangeByXval()");
   draw->Connect("Clicked()","art::TGMWDCCalibration",this,"DoDraw()");
   fBReset->Connect("Clicked()","art::TGMWDCCalibration",this,"Reset()");
   fBSaveSingle->Connect("Clicked()","art::TGMWDCCalibration",this,"Save()");
   fBSaveMWDC->Connect("Clicked()","art::TGMWDCCalibration",this,"SaveMWDC()");

   fComboMWDC->Connect("Selected(Int_t)","art::TGMWDCCalibration",this,"ChangeHist(Int_t)");
   fComboPlane->Connect("Selected(Int_t)","art::TGMWDCCalibration",this,"ChangeHist(Int_t)");
   prev->Connect("Clicked()","art::TGMWDCCalibration",this,"Prev()");
   next->Connect("Clicked()","art::TGMWDCCalibration",this,"Next()");

   fMain->Connect("CloseWindow()","art::TGMWDCCalibration",this,"CloseWindow()");

   {
	 ...
	 if (!fComboMWDC->FindEntry(mwdcname.Data())) fComboMWDC->AddEntry(mwdcname,id);
	 ++id;
      }
      fComboMWDC->Select(0,kFALSE);
      ChangeHist(0);
      Reset();
   }

   const TString &wname = TString("MWDC Plane Calibration");
...
void TGMWDCCalibration::ChangeHist(Int_t id)
{
   fID = id;
...
void TGMWDCCalibration::UpdateComboBox()
{
   const TString& currentmwdcname = CurrentMWDCName();
   const Int_t mwdcid = fComboMWDC->FindEntry(currentmwdcname)->EntryId();

   if (mwdcid != fComboMWDC->GetSelected() || !fComboPlane->GetNumberOfEntries()) {
      Int_t id = 0;
      TIter next(fHists);
   ...
   fComboMWDC->Select(mwdcid,kFALSE);
   fComboPlane->Select(fID,kFALSE);
}

TString TGMWDCCalibration::CurrentMWDCName()
{
   const TString& hname = fHt->GetName();
   return hname(0,hname.Last('_'));
}

TString TGMWDCCalibration::CurrentPlaneName()
{
   const TString& hname = fHt->GetName();
   return hname(hname.Last('_')+1,hname.Length());
}

void TGMWDCCalibration::Prev()
{
   if (!fID) return;
...
void TGMWDCCalibration::CloseWindow()
{
   // Close main frame
   if(fMain) fMain->CloseWindow();
   fMain = NULL;
...
void TGMWDCCalibration::UpdateByNE()
{
...
void TGMWDCCalibration::SaveMWDC()
{
   // Invoke a dialog to save current parameters for MWDC.
   const TString &mwdcname = CurrentMWDCName();
   ...
   const TString &outDir = TString::Format("%s/%s/%s/%s/%s",
					   gSystem->pwd(),fgMWDCParamDir.Data(),
   ...
   if (!gSystem->GetPathInfo(outDir,fileStat)) {
	 ...
	 const TString &msg = TString::Format("Directory \"%s\" exists.\nContinue? (Existing files can be overwritten)",
					      outDir.Data());
	 Int_t ret;
	 new TGMsgBox(gClient->GetDefaultRoot(),fMain,"SaveMWDC",msg.Data(),kMBIconQuestion,kMBYes | kMBNo,&ret);
	 if (ret == kMBNo) return;
      } else {
	 // something exists at outDir
	 const TString &msg = TString::Format("File \"%s\" exists. Abort.",outDir.Data());
	 new TGMsgBox(gClient->GetDefaultRoot(),fMain,"SaveMWDC",msg.Data(),kMBIconExclamation,kMBClose);
	 return;
      }
   } else {
      // outDir not exists
      ...
      Int_t ret;
      new TGMsgBox(gClient->GetDefaultRoot(),fMain,"SaveMWDC",msg.Data(),kMBIconAsterisk,kMBOk | kMBCancel,&ret);
      if (ret == kMBCancel) return;

      if (!gSystem->mkdir(outDir.Data(),kTRUE)) {
	 Info("SaveMWDC","directory %s was created",outDir.Data());
      } else {
	 SysError("SaveMWDC","Failed to make directory at %s",outDir.Data());
	 return;
      }
   }
...
void TGMWDCCalibration::SetRangeByXval()
{
...
void TGMWDCCalibration::GenerateHist()
{
...
void TGMWDCCalibration::SaveToFile(const char* ofilename,Bool_t isAna)
{
