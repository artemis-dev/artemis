/**
 * @file   TPIDwSinglePPACPlaneProcessor.cc
 * @brief  pid with mwdc single plane
 *
 * @date   Created       : 2018-07-12 17:39:49 JST
 *         Last Modified : 2019-11-20 16:00:51 JST (ota)
 * @author Shinsuke OTA <ota@cns.s.u-tokyo.ac.jp>
 *
 *    (C) 2018 Shinsuke OTA
 */

#include "TPIDwSinglePPACPlaneProcessor.h"
#include <TMath.h>
...
using art::TPIDwSinglePPACPlaneProcessor;

ClassImp(TPIDwSinglePPACPlaneProcessor)
...
TPIDwSinglePPACPlaneProcessor::TPIDwSinglePPACPlaneProcessor()
   : fTOFs(NULL), fOutput(NULL)
{
   Register(fPPACParam("PPACParam","name of ppac parameter (single plane)",TString("f5geox")));
   Register(fInput("PlaneName","name of input plane",TString("f5ppacx")));
...
TPIDwSinglePPACPlaneProcessor::TPIDwSinglePPACPlaneProcessor(const TPIDwSinglePPACPlaneProcessor& rhs)
{
}

TPIDwSinglePPACPlaneProcessor& TPIDwSinglePPACPlaneProcessor::operator=(const TPIDwSinglePPACPlaneProcessor& rhs)
{
   if (this != &rhs) {
...
void TPIDwSinglePPACPlaneProcessor::Init(TEventCollection *col)
{
...
void TPIDwSinglePPACPlaneProcessor::Process()
{
   fOutput->Clear("C");
   fOutputZ->Clear("C");
   fBrho1->Clear("C");
   fBrho2->Clear("C");
   if (!(fInput)->GetEntriesFast() ||
...
//         printf("%f %f\n",plane->GetTimestamp(),tof->GetTimestamp());
         ...
         // found
         Double_t x = (plane->GetID()-fPPACParam->GetCenter()) * fPPACParam->GetStripWidth();
...
void TPIDwSinglePPACPlaneProcessor::CalcZAQ(Double_t tof, Double_t x, Double_t& z, Double_t& aq, Double_t &br35, Double_t & br57)
{
