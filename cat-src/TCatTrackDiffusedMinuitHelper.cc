/**
 * @file   TCatTrackDiffusedMinuitHelper.cc
 * @brief  helper for tracking with diffused charge
 *
 * @date   Created       : 2016-07-24 08:12:26 JST
 *         Last Modified : 2016-11-11 23:37:06 JST (ota)
...
using art::TCatTrackDiffusedMinuitHelper;
using TArtSystemOfUnit::deg;

ClassImp(TCatTrackDiffusedMinuitHelper)

TCatTrackDiffusedMinuitHelper::TCatTrackDiffusedMinuitHelper()
: fResponseTable(NULL)
{
   fPulseHelper = NULL;
}
...
TCatTrackDiffusedMinuitHelper::~TCatTrackDiffusedMinuitHelper()
{
   if (fResponseTable) delete fResponseTable;
   if (fPulseHelper) delete fPulseHelper;
}

TCatTrackDiffusedMinuitHelper::TCatTrackDiffusedMinuitHelper(const TCatTrackDiffusedMinuitHelper& rhs)
{
}

TCatTrackDiffusedMinuitHelper& TCatTrackDiffusedMinuitHelper::operator=(const TCatTrackDiffusedMinuitHelper& rhs)
{
   if (this != &rhs) {
...
void TCatTrackDiffusedMinuitHelper::Register(TProcessor *proc)
{
   TCatTrackMinuitHelper::Register(proc);
...
void TCatTrackDiffusedMinuitHelper::Init(TEventCollection *col)
{
   TCatTrackMinuitHelper::Init(col);
   ...
   fPulseHelper = new TClonesArray(TCatPulseShape::Class());
}

void TCatTrackDiffusedMinuitHelper::SetData(TObjArray *input)
{
   fPulseHelper->Clear("C");
...
Double_t TCatTrackDiffusedMinuitHelper::CalcResidual(Double_t x0, Double_t y0, Double_t z0, Double_t vx, Double_t vy, Double_t vz)
{
   // initialize
   Double_t fcn = 0.;
   Double_t fcnE = 0.;
...
//         fcn +=(yExp - yEst2) * (yExp - yEst2) * 0.5 / sigma / sigma;
   ...
   fcn = fcnE + fcnY;
//   Info("CalcResidual","chi2 = %10f",fcn);
...
void TCatTrackDiffusedMinuitHelper::GetResponse(Int_t id, const TVector3 &trackStart, const TVector3& unitDirection, const Double_t &range,
...
void TCatTrackDiffusedMinuitHelper::GetTrackResult(TCatTrackResult *result)
{
   if (!result) return;
...
//   TClonesArray *hits = result->GetHits();
   ...
   Double_t r30max = 0;
#if 0   
   Info("GetTrackResult","New result");
   for (Int_t i=0; i<10; i++) {
      printf("residual[%d] = %10f\n",i,CalcResidual(x0,y0,z0,va.X()+1*(i-5),va.Y(),va.Z()));
   }
#endif   
...
#if 0      
      Info("GetTrackResult","id = %3d exp = %20.5f, fit = %20.10f",id,pulse->GetCharge(),charge);
      GetResponse(id,trackStart,TVector3(unitDirection.X(),unitDirection.Y(),unitDirection.Z()+0.001).Unit(),
                  TMath::Abs(range),nStep,step,charge,closest,rangeToEnd);
      Info("GetTrackResult","id = %3d exp = %20.5f, fit = %20.10f",id,pulse->GetCharge(),charge);
#endif      
      pad->GetIntersection(trackStart,trackEnd,intersection);
...
//======================================================================
//
Bool_t TCatTrackDiffusedMinuitHelper::LoadResponseTable(const char* filename)
{
   ...
   printf("TCatTrackDiffusedMinuit::LoadResponseTable() Loaded\n");
   return kTRUE;
}
